...
@command SkillCancel
@text Skill Cancel
@desc Cancels the skill being used. This command must be called within the skill action common event.
...
@text Character Knockback
@desc Knocks the character back in the specified direction.
...
@command SkillCancel
@text スキルキャンセル
@desc 使用中のスキルをキャンセルします。本コマンドは必ずスキルのアクションコモンイベント内で呼び出してください。
        ...
        *process() {
        ...
        terminate() {
            ...
            if (this._battler.isActor()) {
                if (!this._canceled && !this._battler.isSkillActivating()) {

...
Window_SQSQuestsList = class Window_SQSQuestsList extends Window_Selectable {
  ...
  // * Se estivermos nas missões atuais, isso é usado para verificar se a missão pode ser definida como ativa
  // * ou seja, se ela possui pontos de rota
  isEnabled(quest) {
    if (!quest || quest.id == null) { // Fixed id null error
      return false;
    }
    if (this._group === 0) {
  ...
  drawItem(index) {
    ...
    if (!quest || quest.id == null) { // modified check
      return;
    }
    if (this.index() !== index) {
  ...
  drawQuestActiveSymbol(quest, rect) {
    if (!quest) return; // evita erro se quest for null
